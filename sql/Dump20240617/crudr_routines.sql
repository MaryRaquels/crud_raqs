...
/*!50001 VIEW `relatorio` AS select count(`c`.`idcategoria`) AS `contIDcat`,`c`.`nomecategoria` AS `categoria` from (`produtos` `p` join `categoria` `c` on(`p`.`idcategoria` = `c`.`idcategoria`)) group by `c`.`idcategoria` order by count(`c`.`idcategoria`) desc */;
